...
if [ $MEGA_BACKUP_COUNT -gt 0 ]; then
    for cmd in mega-put mega-ls mega-mkdir; do
        if ! command -v $cmd &> /dev/null; then
            log "Error: $cmd is not installed or not in PATH"
            exit 1
        fi
...
create_and_upload_snapshot() {
    ...
    if [ $MEGA_BACKUP_COUNT -gt 0 ]; then
        ...
        log "MEGA_BACKUP_COUNT is set to 0. Skipping upload to MEGA."
        log "Moving compressed archive to local backup directory..."
        if ! mv "$compressed_archive" "${SCRIPT_DIR}/backup/"; then
            log "Failed to move compressed archive to local backup directory"
            return 1
        fi
        log "Snapshot created and moved to local backup directory successfully."
    fi
}
...
cleanup_old_backups() {
    ...
    # Clean up MEGA backups only if MEGA_BACKUP_COUNT > 0
    if [ $MEGA_BACKUP_COUNT -gt 0 ]; then
        ...
        log "MEGA_BACKUP_COUNT is set to 0. Skipping MEGA backup cleanup."
    fi
}
